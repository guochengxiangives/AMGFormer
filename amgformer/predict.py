...
def softmax_output_dice_class4_1(output, target):
    eps = 1e-8
    ...
    ####### post processing ########
    # 将 o3 转换为 numpy 格式用于后处理
    o3_np = o3.cpu().numpy()

    # 连通区域分析：仅保留大于 500 个体素的区域
    processed_o3 = np.zeros_like(o3_np)
    for b in range(o3_np.shape[0]):  # 遍历 batch
        labeled_mask, num_features = label(o3_np[b] > 0.5)  # 连通区域标记
        for region_idx in range(1, num_features + 1):
            region_size = np.sum(labeled_mask == region_idx)
            if region_size >= 500:  # 仅保留大区域
                processed_o3[b][labeled_mask == region_idx] = 1

    # 形态学操作：开运算去除小伪影，闭运算填补空洞
    structure = np.ones((3, 3, 3))  # 使用 3x3x3 的结构元素
    for b in range(processed_o3.shape[0]):  # 遍历 batch
        processed_o3[b] = binary_opening(processed_o3[b], structure=structure)
        processed_o3[b] = binary_closing(processed_o3[b], structure=structure)

    # 转换回 Tensor 格式
    o4 = torch.from_numpy(processed_o3).float().to(o3.device)

    # 重新计算经过后处理的 Enhancing Dice
    t4 = t3
    intersect4 = torch.sum(2 * (o4 * t4), dim=(1, 2, 3)) + eps
    denominator4 = torch.sum(o4, dim=(1, 2, 3)) + torch.sum(t4, dim=(1, 2, 3)) + eps
    enhancing_dice_postpro = intersect4 / denominator4
...
def visualize_segmentation(pred, target, class_names, slice_idx=0,
                           save_path='/home/renzecheng/gcx/mmformer-root/runWithEAA'):
    """
    Function to visualize a single slice of the segmentation result and optionally save it to a file.

    Parameters:
        pred (torch.Tensor): The predicted segmentation output (probabilities).
        target (torch.Tensor): The ground truth segmentation.
        class_names (list): List of class names.
        slice_idx (int): The index of the slice to visualize.
        save_path (str, optional): Path to save the visualization image. If None, the image will not be saved.
    """
    # Convert to numpy for visualization
    pred_np = pred.cpu().numpy()[0]  # Take the first sample
    target_np = target.cpu().numpy()[0]

    # Choose a slice along the Z-axis (can be modified for other axes)
    ...
    # Plot prediction
    axes[0].imshow(np.argmax(pred_slice, axis=0), cmap='jet', interpolation='none')
    axes[0].set_title("Predicted Segmentation")
    axes[0].axis('off')

    # Plot ground truth
    axes[1].imshow(np.argmax(target_slice, axis=0), cmap='jet', interpolation='none')
...
#         if dataname in ['BRATS2021', 'BRATS2020', 'BRATS2018']:
#             scores_separate, scores_evaluation = softmax_output_dice_class4_1(pred, target)
#         elif dataname == 'BRATS2015':
#             scores_separate, scores_evaluation = softmax_output_dice_class5(pred, target)

#         # Visualization of the segmentation results for the first subject in the batch
#         # visualize_segmentation(pred, target, class_separate, slice_idx=75)
...
def test_softmax(
        ...
        feature_mask=None,
        mask_name=None,
        return_predictions=False):  # 添加新参数以控制返回预测结果
    ...
    ground_truths = [] if return_predictions else None
    ...
    for i, data in enumerate(test_loader):
        ...
        # 存储完整的预测结果（含概率分布）用于可视化
        if return_predictions:
            for batch_idx in range(len(names)):
                # 保存概率分布，用于更详细的可视化
                predictions.append(pred[batch_idx].cpu().numpy())
    ...
    # 如果需要返回预测结果用于可视化，则一并返回
    if return_predictions:
        return vals_evaluation.avg, predictions, ground_truths
    else:
        return vals_evaluation.avg
